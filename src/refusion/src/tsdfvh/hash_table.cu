...
void HashTable::Init(int num_buckets, int bucket_size, int num_blocks,
  ...
  num_entries_ = num_buckets * bucket_size;   // 总线程数
  ...
  heap_->Init(num_blocks);  // 分配堆所需的内存
  int threads_per_block = THREADS_PER_BLOCK;   // 每个block所使用的线程
  int thread_blocks =  // GPU总共使用的block数
