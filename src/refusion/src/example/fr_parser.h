...
class FrParser {
 public:
  /**
   * @brief      Constructs the class.   构造函数，初始化一些参数
   *
 ...
 protected:
  ...
  bool file_opened_ = false;  // 打开文件是否成功
  ...
  int start_frame_;   // 第一帧的位置

  /** The desired final frame */
  int end_frame_;  // 最后一帧的位置

  /** The frame skip */
  int frame_skip_;  // 每次处理完跳 frame_skip_帧
