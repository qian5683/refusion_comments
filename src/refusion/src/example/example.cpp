...
int main(int argc, char const *argv[]) {
  ...
  tsdf_options.voxel_size = 0.01;  // 体素的尺寸0.01 m
  tsdf_options.num_buckets = 50000;
  tsdf_options.bucket_size = 10;
  tsdf_options.num_blocks = 500000;
  tsdf_options.block_size = 8;  // block的尺寸 ，一个block的边=8个voxel_size
  tsdf_options.max_sdf_weight = 64;   // sdf 权重的最大值，权重不会无限累加
  tsdf_options.truncation_distance = 0.1; // 截断距离
  tsdf_options.max_sensor_depth = 5;   // 最大传感器深度
  tsdf_options.min_sensor_depth = 0.1; // 最小传感器深度
  ...
  filepath_out << filebase << "/result.txt";   // 输出位姿结果的文件名
  std::ofstream result(filepath_out.str());   // 输出位姿

  int start = 0;
  int end = -1;
  int frame_skip = 1;
  FrParser parser(start, end, frame_skip);  // 打开文件的构造函数，初始化一些参数
  if (parser.OpenFile(filebase)) { // 打开图像路径文件
    while (parser.ReadNext()) { //读入一帧图像
      tracker.AddScan(parser.GetRgb(), parser.GetDepth());  // 匹配图像，并加到map中
